...
cap = cv2.VideoCapture('http://192.168.43.40:8080/video')       #capturing the video from the droidcam and storing it in cap variable
...
while True:
    ...
    B = error.to_bytes(4, byteorder = 'little', signed = True)
    conn.sendall(B)
    print(error)                         #The final step is to write this error on the Serial Port which
